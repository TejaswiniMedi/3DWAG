...
class Dataset_Build3D_S3(Dataset):
    def __init__(self, dataset_folders, split='train', n_px = 224, wavelet_transform=None, image_transform=None, voxel_transform=None, return_reps=["Voxel_32", "Wavelet"], base_rep="Wavelet", exp_name=None,  args=None, over_write=False, testing_cnt=None):
        ...
        if "sdf_sample" in return_reps:
            self.sdf_points = self.args.sdf_points
            self.sdf_sample_type = self.args.sdf_sample_type
            self.sdf_res = self.args.sdf_res
        ...
        if "Pointcloud" in return_reps:
            self.num_pc_points = self.args.num_pc_points
        ...
        self.args.s3_wavelet_bucket = self.args.s3_bucket
        dataset_folders = self.get_file_s3(self.args.dataset_path, self.args.s3_bucket)

        for dataset_folder in dataset_folders:
            self.set_s3_config()

            wavelet_paths_list_path =  dataset_folders           #os.path.join(wavelet_dataset_mapping[dataset_folder], wavelet_paths_filename)
            ...
            if split == "val":
                ...
                if self.args.train_mode == "finetune":
                    if self.args.ft_train_datasets is not None:
                        if dataset_folder not in self.args.ft_train_datasets:
                           paths = [] 
                    else:    
    ...
    def get_data_filenames(self, args):
        path_file = f'latent_{self.args.point_num}_{self.args.resolution}_{self.args.wavelet}_{self.args.padding_mode}_h5.pkl'
        data_file = f'latent_{self.args.point_num}_{self.args.resolution}_{self.args.wavelet}_{self.args.padding_mode}.h5df'

        if self.args.use_compact_indices:
    ...
    def __getitem__(self, idx):
        ...
        try: 
            ...
            if "sdf" in return_reps:
                wavelet_aug_bucket_name, wavelet_file_path, aug = self.wavelet_aug_apply(self.args.s3_wavelet_bucket, model_path)
                print(wavelet_file_path)
...
####################################################################################################################################################################################################################################################################            
# To check dataset_interface

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    
    parser.add_argument("--sdf_points", type=int, default='10000', help='Number of SDF points')
    parser.add_argument("--sdf_sample_type", type=str, default='uniform', help='uniform, near surface, mix')
    parser.add_argument("--sdf_res", type=int, default=256, help='what is the resolution of SDF')
    #parser.add_argument('--s3_bucket', type=str, default="new-filtered-wavelet-data", help='s3 images_base')
    parser.add_argument('--s3_bucket', type=str, default=sdf_bucket_name, help='s3 images_base')
    ...
    parser.add_argument('--over_write', default=False, help="using overwrite")
    parser.add_argument('--dataset_path', nargs='+' ,  default=["ShapeNet_V2_sdfs"], help='Dataset path')  #, "House3d"]
    parser.add_argument('--max_images_num', type=int, default=40, help='maximum number of images')
    parser.add_argument('--num_pc_points', type=int, default=2048, help='number of points in pc')
    parser.add_argument('--input_view_cnt', type=int, default=1, help='number of ifeatures')
    parser.add_argument("--clip_model_type",  type=str, default='L-14', metavar='N', help='what model to use')

    parser.add_argument("--wavelet_transform",  type=str, default=None, metavar='N', help='wavelet augmentation')
    ...
    parser.add_argument('--ft_train_number', type=int, default=None, help='finetune training number ')
    ...
    def calculate_component_wise_variance_and_mean(volume):
        # Assuming volume is a tensor with shape (b, c, d, h, w)
        volume = torch.from_numpy(volume)
        b, c, d, h, w = volume.shape

        # Calculate the mean
        mu_hat = torch.mean(volume, dim=(0, 2, 3, 4), keepdim=True)  # mean over b, d, h, w
        # Calculate the component-wise variance
        sigma_hat_squared = torch.mean((volume - mu_hat) ** 2, dim=(0, 2, 3, 4), keepdim=True)  # variance over b, d, h, w

        return mu_hat, sigma_hat_squared

    def calculate_mean_and_variance(volume):
        # Assuming volume is a tensor with shape (b, c, d, h, w)
        volume = torch.from_numpy(volume)
        b, c, d, h, w = volume.shape

        # Calculate the mean across c, d, h, w dimensions
        mu_hat = torch.mean(volume, dim=(1, 2, 3, 4), keepdim=True)  # mean over c, d, h, w
        # Calculate the variance across c, d, h, w dimensions
        sigma_hat_squared = torch.mean((volume - mu_hat) ** 2, dim=(1, 2, 3, 4), keepdim=True)  # variance over c, d, h, w

        return mu_hat, sigma_hat_squared

    #"Wavelet", "SDF_GRID_256", "Voxel_16", "image"
    args.exp_name = "Wave_Geometry_Net_all_Wavelet_General_Encoder_Down_2_Wavelet_General_Decoder_Up_2_original_4_1024_1_0.25_256_bior6.8_constant_2_2_2_e_r_0_d_r_0_ema_True_all_batched_threshold_use_sample_training_bf16_1.0_1/"
    dataset = Dataset_Build3D_S3(dataset_folders=args.dataset_path, wavelet_transform=args.wavelet_transform,  return_reps=["sdf"], base_rep="Wavelet",  args=args, image_transform=None, over_write=args.over_write, split='val')
    print(len(dataset))
    for data in dataset:
        print(args.wavelet_transform)
        #print(data['low'].shape)
        #print(data['high_values'].shape)
        #print(data)
        #print(np.expand_dims(data['pre_quant'], axis=0).shape)
        #mean, variance = calculate_component_wise_variance_and_mean(np.expand_dims(data['pre_quant'], axis=0))
        #print(mean.min(), mean.max(), mean.mean(), variance.min(), variance.max(), variance.mean())
        #print(data["Voxel_128"].shape, data["Voxel_64"].shape)
        #print(calculate_mean_and_variance(np.expand_dims(data['pre_quant'], axis=0)))
        break 