...
def get_args_from_config(config):
    ...
    # Parse arguments (override config values if specified in the command line)
...
def build_everything(args: arg_util.Args):
    ...
    config_file = 'config/data_config.yaml'  # Or 'data_config.json'
    config_data = load_config(config_file)

    # Convert the configuration to args
    data_args = get_args_from_config(config_data)
    ...
    if not args.local_debug:
        ...
        dataset_val = data_module.val_dataloader(data_args=data_args)

        dataset_train = data_module.train_dataloader(data_args=data_args)

        ld_val = DataLoader(
            dataset_val, num_workers=0, pin_memory=True,
            batch_size=6, sampler=EvalDistributedSampler(dataset_val, num_replicas=dist.get_world_size(), rank=dist.get_rank()),
        ...
        ld_train = DataLoader(
            ...
            batch_sampler=DistInfiniteBatchSampler(
                dataset_len=len(dataset_train), glb_batch_size=48, same_seed_for_all_ranks=args.same_seed_for_all_ranks,
    ...
    #print("important", args.resos, args.patch_num, args.ls)
...
def main_training():
    ...
    #experiment_comet = Experiment(
    #api_key="7mS85oLdFp7wO7wGbRF5bW0pD",
    #project_name="build3d-gen", 
    #workspace="bernini"
    #)
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, inp in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 2000 else 5, header):
        #print(it,inp,start_it, iters_train,ld_or_itrt)
        #label = None
        ...
        #label = generate_random_labels(low.shape[0])
        label = None
