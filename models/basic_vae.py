...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, dropout): # conv_shortcut=False,  # conv_shortcut: always False in VAE
        ...
        self.dropout = torch.nn.Dropout3d(dropout) if dropout > 1e-6 else nn.Identity()
...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        B, _, H, W, D = qkv.shape  # should be B,3C,H,W,D
        C = self.C
        q, k, v = qkv.reshape(B, 3, C, H, W, D).unbind(1)
        
        # compute attention
        q = q.view(B, C, H * W * D).contiguous()
        q = q.permute(0, 2, 1).contiguous()     # B,HWD,C
        k = k.view(B, C, H * W * D).contiguous()    # B,C,HWD
        w = torch.bmm(q, k).mul_(self.w_ratio)  # B,HWD,HWD    w[B,i,j]=sum_c q[B,i,C]k[B,C,j]
        w = F.softmax(w, dim=2)
        
        # attend to values
        v = v.view(B, C, H * W * D).contiguous()
        w = w.permute(0, 2, 1).contiguous()  # B,HWD,HWD (first HW of k, second of q)
        h = torch.bmm(v, w)  # B, C,HWD (HWD of q) h[B,C,j] = sum_i v[B,C,i] w[B,i,j]
        h = h.view(B, C, H, W, D).contiguous()
...
class Decoder(nn.Module):
    def __init__(
        self, *, ch=128, ch_mult=(1, 2, 4, 8), num_res_blocks=2,
        dropout=0.0, in_channels=1,  # in_channels: raw voxel channels
        ...
        # end
        self.norm_out = Normalize(block_in)
        self.conv_out = torch.nn.Conv3d(block_in, in_channels, kernel_size=3, stride=1, padding=2)       # 1-----> 2
