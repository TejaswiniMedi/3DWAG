...
try:
    ...
    sys.path.append("external/DCUDF")
    sys.path.append("../external/DCUDF")
    sys.path.append("../../external/DCUDF")
    from dcudf.mesh_extraction import dcudf
except Exception as e:
    print("Cannot import DCUDF")
    print(e)
...
# v_sdf: (1, 1, 256,256,256) torch tensor
# return: low, highs
# low: (1, 1, 46, 46, 46)
...
# v_sdf: (1, 1, 256,256,256) torch tensor
# return: Volume
# Volume: (1, 64(or 8), 46, 46, 46)
def sdf2volume(v_sdf, depth=2):
    low, highs = sdf2wavelets(v_sdf, 3)
    if depth == 1:
        volume = torch.cat((low[None], highs[2]), dim=2)[0]
    else:
        padded_d1 = F.pad(highs[1], (8,8,8,8,8,8), 'constant', 0)
        padded_d1 = torch.cat(
            (
                padded_d1[:, :, :, 0::2, 0::2, 0::2],
                padded_d1[:, :, :, 0::2, 0::2, 1::2],
                padded_d1[:, :, :, 0::2, 1::2, 0::2],
                padded_d1[:, :, :, 0::2, 1::2, 1::2],
                padded_d1[:, :, :, 1::2, 0::2, 0::2],
                padded_d1[:, :, :, 1::2, 0::2, 1::2],
                padded_d1[:, :, :, 1::2, 1::2, 0::2],
                padded_d1[:, :, :, 1::2, 1::2, 1::2],
            ),
            dim=2,
        )
        volume = torch.cat((low[None], highs[2], padded_d1), dim=2)[0]
    return volume


def volume2wavearray(volume, depth=2):
    lows = volume[None, :, 0]
    d0 = volume[None, :, 1:8]
    if depth == 1:
        d1 = torch.zeros((1, 1, 7, 76, 76, 76)).to(volume.device)
        d2 = torch.zeros((1, 1, 7, 136, 136, 136)).to(volume.device)
    else:
        d1_ori = volume[:, 8:]
        d1 = torch.zeros((1, 1, 7, 92, 92, 92)).to(volume.device)
        d1[:, :, :, 0::2, 0::2, 0::2] = d1_ori[:, :7]
        d1[:, :, :, 0::2, 0::2, 1::2] = d1_ori[:, 7:14]
        d1[:, :, :, 0::2, 1::2, 0::2] = d1_ori[:, 14:21]
        d1[:, :, :, 0::2, 1::2, 1::2] = d1_ori[:, 21:28]
        d1[:, :, :, 1::2, 0::2, 0::2] = d1_ori[:, 28:35]
        d1[:, :, :, 1::2, 0::2, 1::2] = d1_ori[:, 35:42]
        d1[:, :, :, 1::2, 1::2, 0::2] = d1_ori[:, 42:49]
        d1[:, :, :, 1::2, 1::2, 1::2] = d1_ori[:, 49:56]
        d1 = d1[:, :, :, 8:-8, 8:-8, 8:-8]
        d2 = torch.zeros((1, 1, 7, 136, 136, 136)).to(volume.device)
    return lows, [d2, d1, d0]


def volume2sdf(volume, depth=2):
    return wavelets2sdf(*volume2wavearray(volume, depth), 3)[0,0].cpu().numpy()
...
# v_sdf: (256,256,256) np.ndarray
# return: wavelet volume: (46,46,46,15) np.ndarray
def sdf2volume_yl(v_sdf):
    coeffs = pywt.wavedecn(v_sdf, "bior6.8", level=3)
    low = coeffs[0][...,None]
    high2 = coeffs[1]
    high1 = coeffs[2]
    high0 = coeffs[3]
    high2 = np.stack([
        high2["aad"],high2["ada"],high2["add"],
        high2["daa"],high2["dad"],high2["dda"],high2["ddd"],
    ], axis=3)
    high1 = np.stack([
        high1["aad"],high1["ada"],high1["add"],
        high1["daa"],high1["dad"],high1["dda"],high1["ddd"],
    ], axis=3)
    
    volume = np.concatenate([
        low, high2, high1[15:-15,15:-15,15:-15]
    ], axis=3)

    return volume

# wavelet volume: (46,46,46,15) np.ndarray
# return: v_sdf: (256,256,256) np.ndarray
def volume2sdf_yl(v_volume):
    lows = torch.from_numpy(v_volume[..., 0])[None, None]
    print(v_volume.shape)

    high2 = np.zeros((136,136,136,7), dtype=np.float32)
    high1 = np.zeros((76,76,76,7), dtype=np.float32)
    high1[15:-15,15:-15,15:-15] = v_volume[...,8:]
    high0 = v_volume[...,1:8]

    highs = [
        torch.from_numpy(high2).permute(3,0,1,2)[None, None],
        torch.from_numpy(high1).permute(3,0,1,2)[None, None],
        torch.from_numpy(high0).permute(3,0,1,2)[None, None],
    ]
    sdf = wavelets2sdf(lows, highs, 3)[0,0].cpu().numpy()
    return sdf
...
def udf2mesh(v_udf, v_threshold=0.01, v_batch=800000):
    resolution = v_udf.shape[0]
    if False:
        ...
        v_udf = torch.from_numpy(v_udf).float().to(device)[None,None]
        def query_func(pts):
            return torch.nn.functional.grid_sample(
                v_udf, 
                pts[None,None,None])[0,0,0]
        module = dcudf(query_func, resolution, v_threshold,
                        max_iter=400, is_cut=False,max_batch=v_batch, 
                        report_freq=400, watertight_separate=False,laplacian_weight=0)
        mesh = module.optimize()
