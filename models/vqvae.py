...
class VQVAE(nn.Module):
    def __init__(
        ...
        v_patch_nums=(1, 2, 3, 4),      #, 5, 6, 8),        #, 5, 6, 8, 6, 8, 10, 13, 16), # number of patches for each scale, h_{1 to K} = w_{1 to K} = v_patch_nums[k]
        test_mode=False,
    ):
        super().__init__()
    ...
    def forward(self, inp, ret_usages=True):   # -> rec_B3HW, idx_N, loss
        ...
        inp = inp.unsqueeze(1)
        #vertices, triangles = mcubes.marching_cubes(inp.cpu().detach().numpy()[0, 0], 0.0)
        #vertices = (vertices / 64) * 2.0 - 1.0
        #mcubes.export_obj(vertices, triangles, f'results/test.obj')
